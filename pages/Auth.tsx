  ...
  return (
    <div className="mt-3 space-y-1.5">
      <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-gray-400">
        ...
        <span className={`${strength < 2 ? 'text-red-500' : strength < 4 ? 'text-yellow-600' : 'text-green-600'}`}>{label}</span>
      </div>
...
const PasswordRequirements: React.FC<{ password: string }> = ({ password }) => {
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-3">
      {reqs.map((req, i) => (
        <span key={i} className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full transition-colors duration-300 ${req.met ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
          {req.met ? <Check size={10} className="stroke-[3]" /> : <div className="w-2 h-2 rounded-full border border-gray-300"></div>}
          {req.label}
        </span>
      ))}
    </div>
...
const AuthLayout: React.FC<{ children: React.ReactNode; title: string; subtitle: string; image: string }> = ({ children, title, subtitle, image }) => (
  <div className="min-h-screen flex bg-white font-sans">
    ...
    <div className="w-full lg:w-[55%] flex flex-col justify-center p-6 lg:p-12 xl:p-20 relative overflow-y-auto">
      <div className="max-w-xl w-full mx-auto space-y-8 animate-in slide-in-from-left-4 duration-700">
        ...
        <div className="text-center lg:text-left">
          <Link to="/" className="inline-flex items-center gap-2 text-leaf-600 font-extrabold text-2xl mb-8 tracking-tight hover:scale-105 transition-transform">
            <div className="bg-leaf-50 p-2 rounded-xl"><Leaf className="fill-leaf-600" size={24} /></div>
            FreshLeaf
          </Link>
          <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-3 tracking-tight leading-[1.1]">
            {title}
          </h1>
      ...
      {/* Decorative Elements */}
      <div className="absolute top-0 left-0 w-64 h-64 bg-green-50 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-50"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-yellow-50 rounded-full blur-3xl translate-x-1/3 translate-y-1/3 pointer-events-none opacity-50"></div>
    </div>
...
export const Login: React.FC = () => {
  ...
  return (
    ...
    >
      {/* Auth Method Toggle */}
      <div className="bg-gray-50 p-1 rounded-2xl inline-flex w-full mb-4 border border-gray-100">
        <button 
          onClick={() => setAuthMethod('email')}
          className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${authMethod === 'email' ? 'bg-white text-gray-900 shadow-sm border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}
        >
          <Mail size={16}/> Email
        </button>
        <button 
          onClick={() => setAuthMethod('phone')}
          className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${authMethod === 'phone' ? 'bg-white text-gray-900 shadow-sm border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}
        >
      ...
      {authMethod === 'email' ? (
        <form onSubmit={handleEmailLogin} className="space-y-5">
          <div>
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-leaf-500/50 focus:border-leaf-500 transition-all font-medium focus:bg-white"
              />
            </div>
          </div>
          <div>
            ...
            <div className="relative group">
              ...
              <input 
                ...
                placeholder="••••••••"
                className="w-full pl-12 pr-12 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-leaf-500/50 focus:border-leaf-500 transition-all font-medium focus:bg-white"
              />
              <button 
                type="button"
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full bg-gray-900 hover:bg-leaf-600 text-white py-4 rounded-xl font-bold text-lg shadow-xl hover:shadow-leaf-500/30 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group"
          >
        ...
        <form onSubmit={showOtpInput ? handleVerifyOtp : handleSendOtp} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full pl-16 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-leaf-500/50 focus:border-leaf-500 transition-all font-bold text-lg tracking-widest focus:bg-white"
          ...
          <button 
            ...
            className="w-full bg-gray-900 hover:bg-leaf-600 text-white py-4 rounded-xl font-bold text-lg shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group"
          >
      ...
      <form onSubmit={handleSignup} className="space-y-6">
        ...
        <div className="flex justify-center mb-6">
          <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
            <div className="w-28 h-28 rounded-full border-4 border-white shadow-xl overflow-hidden bg-gray-100 flex items-center justify-center hover:border-leaf-200 transition-all ring-2 ring-transparent hover:ring-leaf-200">
              {profileImage ? (
          ...
          <div>
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-leaf-500/50 focus:border-leaf-500 transition-all font-medium text-sm focus:bg-white"
              />
            </div>
          </div>
          <div>
            ...
            <select 
              ...
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-leaf-500/50 focus:border-leaf-500 transition-all font-medium text-sm text-gray-700 appearance-none focus:bg-white"
            >
        ...
        <div className="space-y-5">
          ...
          <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100">
            ...
            <div className="relative group mt-4 pt-4 border-t border-gray-200">
              ...
              <div className="relative">
                ...
                <input 
                    ...
                    className={`w-full pl-11 pr-10 py-3 bg-white border rounded-xl focus:outline-none focus:ring-2 transition-all font-medium text-sm ${
                        formData.confirmPassword && formData.password !== formData.confirmPassword 
                        ? 'border-red-300 focus:border-red-500 focus:ring-red-100' 
                        : 'border-gray-200 focus:border-leaf-500 focus:ring-leaf-500/50'
                    }`}
                />
                {formData.confirmPassword && formData.password === formData.confirmPassword && (
        ...
        <button 
          ...
          className="w-full bg-leaf-600 hover:bg-leaf-700 text-white py-4 rounded-xl font-bold text-lg shadow-xl shadow-leaf-200 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group"
        >
